...
10. 范围冻结：超出 MVP 的能力全部进入 backlog，不在当前迭代实现。
...
2. 错误码可稳定返回并可观测。
3. EnterTicket 必须真实执行 HMAC/JWT 签名校验，错误密钥和字段篡改必须拒绝。
4. nonce 重复消费必须返回重放错误，不能只验证正向登录链路。
...
1. 慢客户端场景不拖垮整体服务。
2. 重连流程稳定，状态恢复正确。
3. 合法业务消息和 `heartbeat_req` 都能刷新连接期限，非法消息不能续期。
4. 长连接保持测试必须超过两倍连接超时时间，不能用短时 smoke 代替。
